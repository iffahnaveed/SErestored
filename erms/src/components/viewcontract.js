...
// import './sendmsghr.css'; // Optional: reuse styles
...
//     axios.get(`http://localhost:5000/api/jobid/${selectedApplicationId}`)
//       .then(res => {
...
//     axios.get(`http://localhost:5000/api/contract/${jobId}`)
//       .then(res => {
...
//           ))}
//         </select>
//         <button type="button" onClick={handleLoadJobId}>Load Job ID</button>
//       </div>
...
//       <button type="button" onClick={handleViewContract}>View Contract</button>

//       {contract && (
//         <div style={{ marginTop: '20px' }}>
//           <h3>Contract Details</h3>
//           <p><strong>Contract ID:</strong> {contract.contract_id}</p>
//           <p><strong>HR ID:</strong> {contract.hr_id}</p>
//           <p><strong>Salary:</strong> {contract.salary}</p>
//           <p><strong>Probation Period:</strong> {contract.probation_period}</p>
//           <p><strong>Start Date:</strong> {new Date(contract.hr_contractstart_date).toLocaleDateString()}</p>
//           <p><strong>End Date:</strong> {new Date(contract.hr_contractend_date).toLocaleDateString()}</p>
...
const ViewContract = () => {
  ...
  const handleLoadJobId = () => {
    ...
    axios
      .get(`http://localhost:5000/api/jobid/${selectedApplicationId}`)
      .then((res) => {
        setJobId(res.data.jobId);
        console.log('Loaded Job ID:', res.data.jobId);
      })
  ...
  const handleViewContract = () => {
    ...
    axios
      .get(`http://localhost:5000/api/contract/${jobId}`)
      .then((res) => {
        setContract(res.data);
        console.log('Contract details:', res.data);
      })
